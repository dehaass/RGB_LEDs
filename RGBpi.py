...
def defineCol(ser):
    ...
    for colour in RGB:
        while True:
            try:
                val = int(input("How much %s? (0-255): " %colour))
            except ValueError:
                print("Invalid input")
                val = 300
            if val > 255 or val < 0:
...
def sleepArd(ser):
    writeArd(ser, COMMANDS['SLEEP'])

def manualControl(ser):

    while True:
        try:
            cmd = int(input("Command (one byte): "))
            if cmd > 255 or cmd < 0:
...
def doStuff(ser):
    ...
    while True:
        ...
        cmd = input("What's your command Master? ")
        flushSer(ser)
        if cmd == 'q':
...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "mhalc:s:")
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            changeSettings(ser, [-1, 250, 1, 1, 2, -1, -1, -1, 50, 9])
            return
        elif opt == '-l':
            sleepArd(ser)
            return
